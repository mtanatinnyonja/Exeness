"""
Agent Trading Principal.
Flux: broker MT5/Exness → signaux → filtre mémoire → IA → ordre/paper trade.
"""
...
from config import (
    ...
    SYMBOL_SOURCE_MODE, PREFERRED_SYMBOLS, ALLOW_TRADE_EXECUTION
)
...
class TradingAgent:
    def __init__(self, quiet: bool = False):
        ...
        self.analyst = ClaudeAnalyst(self.memory)
        self.last_signal_time = {}

        if not quiet:
            self.memory.log_session(
                f"🚀 Agent démarré | broker={getattr(self.broker, 'name', 'unknown')} | ia={self.analyst.provider}"
            )
    ...
    def run_cycle(self):
        ...
        self.memory.log_session(f"📡 Symboles MT5 actifs: {', '.join(instruments)}")
    ...
    def _analyze_instrument(self, instrument: str, account: Dict):
        ...
        try:
            ...
            if score < MIN_SIGNAL_SCORE or direction is None:
                self.store.record_signal_sample(instrument, signal_h1, 0.0, {"decision": "WAIT", "confidence": 0.0})
                self.memory.log_session(f"🔇 {instrument}: score trop faible ({score})")
                return
            ...
            decision = self.analyst.analyze_signal(instrument, signal_h1, account, market_ctx)
            if not decision:
                return
    ...
    def _execute_trade(self, instrument: str, decision: Dict, signal: Dict, account: Dict):
        ...
        order = self.broker.place_market_order(
            ...
            comment=f"AI|{signal.get('pattern', '')[:20]}|{int(confidence * 100)}"
        )
    ...
    def preview_ai_decision(self, instrument: str = None) -> Dict:
        ...
        target = instrument or (symbols[0] if symbols else INSTRUMENTS[0])
        account = self.broker.get_account_summary()
        ...
        decision = self.analyst.analyze_signal(target, signal, account, context)
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "api_calls_today": self.memory.get_api_calls_today(),
            ...
            "ai_provider": self.analyst.provider,
