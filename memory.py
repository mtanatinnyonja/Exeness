...
Apprend des erreurs, du coût tokens, des patterns faibles et des setups gagnants.
"""
...
from config import MEMORY_FILE, TRADES_FILE, DAILY_TOKEN_BUDGET
...
class AgentMemory:
    ...
    def increment_api_calls(self) -> int:
        ...
        if self.memory["api_calls_date"] != today:
            self.memory["api_calls_today"] = 0
            self.memory["api_calls_date"] = today
        self.memory["api_calls_today"] += 1
        self.save()
        return self.memory["api_calls_today"]
...
Appels API aujourd'hui: {self.get_api_calls_today()}
...
"""
    ...
    def add_claude_insight(self, insight: str):
        self.memory.setdefault("last_claude_insights", []).append(
            f"[{datetime.utcnow().strftime('%Y-%m-%d %H:%M')}] {insight}"
        )
        self.memory["last_claude_insights"] = self.memory["last_claude_insights"][-20:]
        self.save()
