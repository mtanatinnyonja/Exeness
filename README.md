# 🤖 Exeness — Bot de trading multi-agent (MT5 + LLM local optionnel)

Exeness est un bot de trading Python **100 % local** composé de 5 agents IA autonomes qui communiquent via un bus de messages asynchrone. Il se connecte à MetaTrader 5, inclut un dashboard web, et envoie des notifications Telegram enrichies.
...
```
============================================================
  ⚡  EXENESS — Système Multi-Agents
============================================================
🌐  Dashboard  →  http://localhost:8765

🤖 AGENTS IA AUTONOMES — DÉMARRAGE
====================================================================
...
|---|---|---|
| `preferred_symbols` | Paires à trader (ex. `EURUSDm,XAUUSDm`) | Toutes les paires visibles dans MT5 |
| `max_risk_per_trade` | Risque max en % du capital par trade | `0.02` (2 %) |
| `daily_target` | Objectif de gain journalier en USD | `2.0` |
| `daily_loss_limit` | Limite de perte journalière en USD (négatif) | `-5.0` |
| `max_open_positions` | Nombre max de positions simultanées | `3` |
...
Le backtest charge les données historiques depuis MT5 et simule le comportement du bot.

```bash
python backtest.py EURUSD 30
```
...
# BTC sur 7 jours
python backtest.py BTCUSDm 7
```

> Le nom de l'instrument doit correspondre exactement à celui affiché dans MT5 (ex. `EURUSDm` pour les brokers Exeness).
...
```bash
python optimizer.py EURUSDm 30
```
...
| Agent | Rôle |
|---|---|
| **AnalystAgent** | Analyse le marché toutes les 30s, publie des signaux (score 1-5) |
...
| Spread guard | Rejet si spread trop élevé (XAU > 35p, BTC > 80p, Forex > 5p) |
...
2. Vérifier dans le dashboard que des paires sont bien visibles dans Market Watch MT5.
